...
- Body/labels/buttons switch from Manrope to a cleaner, more neutral sans (Inter-class) for a restrained financial-services feel
...
- **"View LinkedIn" buttons:** navy background, white text, hover shifts to secondary navy with a subtle gold underline/border accent.
- **Section titles & icons:** dark navy headings; the small gold rules under titles stay but shift to the new gold tone.
- **Stats band:** numbers in navy, labels in secondary text, gold dividers kept subtle.
- **Skill chips & cards:** white cards, `#E5E7EB` hairline borders, softer/flatter shadows, hover becomes gentle elevation with a navy/gold tint instead of indigo.
...
- All work is token edits in `src/styles.css` plus class-name swaps in `src/routes/index.tsx` (indigo/primary utilities → navy/gold tokens). Body font link updated in `src/routes/__root.tsx`. No schema, server-function, or route changes.
